...
// ---------------------------------------------------------------------------
// Livox SDK config file generation (same pattern as mid360_native)
// ---------------------------------------------------------------------------
...
static std::string write_sdk_config(const std::string& host_ip,
    ...
    char tmpl[] = "/tmp/livox_fastlio2_XXXXXX";
    int fd = mkstemp(tmpl);
    if (fd < 0) {
        perror("mkstemp");
        return "";
...
int main(int argc, char** argv) {
    ...
    // Write Livox SDK config
    std::string sdk_config_path = write_sdk_config(host_ip, lidar_ip, ports);
    ...
    if (!LivoxLidarSdkInit(sdk_config_path.c_str(), host_ip.c_str())) {
        ...
        std::remove(sdk_config_path.c_str());
        return 1;
    }
