...
"""
Force-Torque Pull Skill Module with xArm Driver Integration

This module is identical to ft_pull_skill.py but sends joint commands to the
xArm driver via LCM instead of directly calling the xArm SDK.

Key differences from ft_pull_skill.py:
- Subscribes to joint_state from xArm driver for current positions
- Publishes joint_position_command to xArm driver instead of calling SDK
- No direct xArm SDK dependency (uses driver via LCM)
"""
...
from dimos.core import Module, In, Out, rpc, LCMTransport
...
class FTPullXArmModule(Module):
    """Force-Torque based pull skill module for door opening with xArm driver integration."""
    ...
    # Input port for joint state from xArm driver
    joint_state_input: In[JointState] = None  # Joint state from xArm driver

    # Output port for joint commands to xArm driver
    joint_command_output: Out[JointCommand] = None  # Joint commands to xArm driver

    def __init__(
        self,
        xarm_ip: str = None,
        joint_command_channel: str = "/xarm/joint_position_command",
        joint_state_channel: str = "/xarm/joint_state",
        control_rate: float = 5.0,
        verbose: bool = False,
    ):
        """
        Initialize the FT pull module with xArm driver integration.

        Args:
            xarm_ip: IP address of the xARM robot (used for initial position query)
            joint_command_channel: LCM channel for joint commands
            joint_state_channel: LCM channel for joint state
            control_rate: Control loop rate in Hz (default: 5.0)
        ...
        # Joint state from xArm driver
        self.latest_joint_state = None
        self.joint_state_lock = threading.Lock()

        # xARM initial positions (from direct query)
    ...
    def start(self):
        ...
        logger.info("Starting FT Pull (xArm Driver) module")
        ...
        # Subscribe to joint state from xArm driver (transport set from deployment script)
        if self.joint_state_input:
            self.joint_state_input.subscribe(self.handle_joint_state)
            logger.info(f"Subscribed to joint state from xArm driver: {self.joint_state_channel}")

        if self.joint_command_output:
            logger.info(f"Publishing joint commands to xArm driver: {self.joint_command_channel}")
    ...
    def handle_joint_state(self, msg: JointState):
        """Handle incoming joint state from xArm driver."""
        with self.joint_state_lock:
            self.latest_joint_state = msg
            if self.verbose:
                logger.debug(f"Joint state: {[f'{p:.3f}' for p in msg.position]}")

    def get_xarm_positions(self):
        """Get current joint positions from xARM robot via direct SDK query."""
        try:
            from xarm.wrapper import XArmAPI

            logger.info(f"Querying initial positions from xArm at {self.xarm_ip}")
    ...
    def execute_motion(self, target_pose: RigidTransform) -> bool:
        """
        Execute motion using differential IK and send commands to xArm driver.
        """
        ...
        if result.status == DifferentialInverseKinematicsStatus.kSolutionFound:
            ...
            # Send command to xArm driver via LCM
            self.command_xarm_driver(q_new)

            return True
        else:
            return False

    def command_xarm_driver(self, positions: np.ndarray):
        """Send joint position commands to xArm driver via LCM."""
        # Extract arm joint positions (first 6 joints)
        arm_joint_names = [f"joint{i + 1}" for i in range(6)]
        joint_positions = []
        ...
        for joint_name in arm_joint_names:
            try:
                ...
                joint_positions.append(positions[joint_idx])
            except:
                logger.warning(f"Could not find joint {joint_name}")

        # Create and publish JointCommand message
        ...
        try:
            self.joint_command_output.publish(cmd_msg)
            if self.verbose:
                logger.debug(f"Sent joint command: {[f'{p:.3f}' for p in joint_positions]}")
        except Exception as e:
    ...
    def continuous_pull(
        ...
        Sends commands to xArm driver via LCM.
        ...
        """
        logger.info(f"Starting continuous adaptive door pulling (xArm Driver mode)")
        ...
        control_rate = self.control_rate  # Use configured control rate
        dt = 1.0 / control_rate

        logger.info(f"  Control rate: {control_rate}Hz (dt={dt * 1000:.1f}ms per step)")
        ...
        while self.running:
            ...
            if self.plant and (np.linalg.norm(translation) > 0.001 or abs(rotation_angle) > 0.001):
                ...
                # Execute motion (sends command to xArm driver)
                success = self.execute_motion(target_pose)
                if success:
                    self.motion_count += 1
                    self.last_motion_time = time.time()
    ...
    def cleanup(self):
        """Clean up resources."""
        logger.info("FTPullXArmModule cleanup complete")
