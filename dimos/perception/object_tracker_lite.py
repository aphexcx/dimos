...
"""Lightweight object tracker without heavy model dependencies."""
...
class ObjectTrackingStreamLite:
    ...
    def __init__(
        ...
        """
        Initialize a lightweight object tracking stream.

        Args:
            camera_intrinsics: List in format [fx, fy, cx, cy]
    ...
    def track(self, bbox, frame=None, distance=None, size=None):
        ...
        Args:
            ...
            frame: Optional - Current frame (not used in lite version)
        ...
        if self.distance_estimator is not None:
            if size is not None:
                ...
                print("No distance or size provided. Using default object size.")
    ...
    def create_stream(self, video_stream: Observable) -> Observable:
        """Create an Observable stream of object tracking results."""
        ...
        def process_frame(frame):
            ...
            if self.tracker is not None and self.tracking_bbox is not None:
                if not self.tracking_initialized:
                    ...
                    if roi.size > 0:
                        ...
                        if self.original_des is None:
                            print("Warning: No ORB features found in initial ROI.")
                        else:
                        ...
                        print("Error: Empty ROI during tracker initialization.")
                        self.stop_track()
            ...
            elif self.tracking_initialized:  # Tracking stopped this frame
                self.stop_track()  # Reset tracker state
