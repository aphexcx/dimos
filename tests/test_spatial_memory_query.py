...
def parse_args():
    ...
    parser.add_argument(
        "--query", type=str, default=None, help="Text query to search for (e.g., 'kitchen table')"
    )
    ...
    parser.add_argument("--save-all", action="store_true", help="Save all result images")
    parser.add_argument("--save-one", action="store_true", help="Save only the best matching image")
    parser.add_argument(
        "--visualize",
        action="store_true",
...
def main():
    ...
    spatial_memory = SpatialMemory(
        collection_name="test_spatial_memory", chroma_client=db_client, visual_memory=visual_memory
    )
    ...
    if args.visualize:
        ...
        visualize_spatial_memory_with_objects(
            spatial_memory, objects=common_objects, output_filename="spatial_memory_map.png"
        )
    ...
    if args.query:
        ...
        if args.threshold is not None:
            ...
            for result in results:
                ...
                similarity = 1.0 - (
                    result.get("distance", 0) if result.get("distance") is not None else 0
                )
